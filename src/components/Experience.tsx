...
const Experience: React.FC = () => {
  return (
    <section id="experience" className="relative py-20 transition-colors duration-300 ease-in-out">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-2xl mx-auto">
          <h2 className="text-3xl sm:text-4xl font-semibold text-slate-900 dark:text-slate-50">Immersive Viewing Experience</h2>
          <p className="mt-3 text-slate-600 dark:text-slate-300">A minimal viewer with gentle parallax, subtle zoom, and intuitive controls designed for motion art.</p>
        </div>
        ...
        <div className="mt-10 grid lg:grid-cols-2 gap-10 items-center">
          <motion.div
            ...
            className="rounded-2xl p-[2px] bg-gradient-to-br from-white/70 to-white/30 dark:from-white/10 dark:to-white/5"
          >
            <div className="rounded-2xl overflow-hidden backdrop-blur-xl bg-white/70 dark:bg-slate-900/80 border border-slate-200/70 dark:border-slate-700/60">
              <div className="relative h-72 sm:h-96 overflow-hidden">
                <motion.div
                  className="absolute inset-0 bg-gradient-to-br from-fuchsia-500 via-sky-500 to-teal-400"
                  initial={{ scale: 1.05 }}
                  whileInView={{ scale: 1 }}
          ...
          <div className="grid sm:grid-cols-3 gap-6">
            {features.map((f, i) => (
              <motion.div key={f.title} initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.35, ease: 'easeOut', delay: i * 0.05 }} className="rounded-xl border border-slate-200/70 dark:border-slate-700/60 bg-white/70 dark:bg-slate-900/70 backdrop-blur-md p-5">
                <h4 className="font-semibold text-slate-900 dark:text-slate-50">{f.title}</h4>
